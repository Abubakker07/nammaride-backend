...
// --- CRITICAL FIX 1: Prevent PHP warnings from corrupting the JSON ---
error_reporting(0);

require 'db.php';

$route_id = isset($_GET['route_id']) && is_numeric($_GET['route_id']) ? (int)$_GET['route_id'] : 0;

// --- CRITICAL FIX 2: Return safe empty arrays instead of error strings ---
if ($route_id === 0) {
    echo json_encode(["status" => "error", "route_id" => 0, "options" => []]);
    exit;
}
...
try {
    ...
    if (!$route) {
        echo json_encode(["status" => "error", "route_id" => $route_id, "options" => []]);
        exit;
    }
    ...
    // Prepare Driver Query outside the loop for high performance
    ...
    $sorted_fares = empty($fare_results) ? [] : quickSortFares($fare_results);
    ...
    // --- CRITICAL FIX 3: Catch DB errors and return a safe empty array ---
    echo json_encode([
        "status" => "error",
        "route_id" => $route_id,
        "options" => []
